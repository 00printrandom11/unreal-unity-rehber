import { componentData, variableData, nodeData, shortcutData } from '../unityData.js';
import { unityGuideData } from '../unityGuideData.js';

export class UnityContentArea {
    ...
    renderComponents() {
        ...
        searchBox.innerHTML = `
            <div class="actor-search-wrapper">
                ...
                <input type="text" id="actor-search-input" class="actor-search-input" placeholder="Component ara..." autocomplete="off" />
            </div>
        `;
        ...
        this.buildComponentTree(componentData, treeContainer, 0);
        ...
        const totalComponents = countNodes(componentData);

        detail.innerHTML = `
            <div class="actor-welcome">
                <h2>🧩 Component Hiyerarşisi</h2>
                <p>Soldaki arama kutusunu kullanarak veya ağaçtan bir bileşen seçerek detayları görüntüleyin.</p>
                <div class="actor-stats">
                    <div class="actor-stat-item">
                        <span class="actor-stat-number">${componentData.length}</span>
                        <span class="actor-stat-label">Ana sınıf</span>
                    </div>
                    <div class="actor-stat-item">
                        <span class="actor-stat-number">${totalComponents}</span>
                        <span class="actor-stat-label">Toplam bileşen</span>
                    </div>
                </div>
            </div>
        `;
        ...
        this._initComponentSearch();
    }

    buildComponentTree(nodes, parent, depth) {
        ...
        nodes.forEach(node => {
            ...
            if (hasChildren) {
                ...
                this.buildComponentTree(node.children, childrenEl, depth + 1);
    ...
    showComponentDetail(node) {
        ...
        if (node.inspectorNotes) {
            inspectorNotesHtml = `<div class="actor-section actor-bp-notes">
                <h3 class="actor-section-title">🔷 Inspector Notları</h3>
                <p>${(node.inspectorNotes || '').replace(/\n/g, '<br>')}</p>
            </div>`;
        }
    ...
    renderSearch() {
        ...
        input.placeholder = 'Unity API Ara... (Örn: Transform, Rigidbody)';
        input.focus();
    ...
    highlightCSharp(code) {
        ...
        if (code.includes('<span class="c')) return code;
        const keywords = new Set([
            'using', 'public', 'private', 'protected', 'void', 'return', 'if', 'else', 'for', 'foreach', 'while',
            'new', 'class', 'static', 'override', 'virtual', 'abstract', 'true', 'false', 'null', 'this', 'base',
            'get', 'set', 'var', 'int', 'float', 'bool', 'string', 'switch', 'case', 'break', 'default', 'try',
            'catch', 'finally', 'throw', 'async', 'await', 'yield'
        ]);
        const types = new Set([
            'MonoBehaviour', 'GameObject', 'Transform', 'Vector3', 'Vector2', 'Quaternion', 'Rigidbody', 'Collider',
            'Camera', 'Color', 'List', 'Dictionary', 'IEnumerator', 'WaitForSeconds', 'Coroutine', 'NavMeshAgent',
            'Animator', 'AudioSource', 'Canvas', 'RectTransform', 'ScriptableObject', 'SerializeField', 'Header',
            'Tooltip', 'Range', 'RequireComponent'
        ]);
        ...
        while (i < len) {
            ...
            if (code.slice(i, i + 2) === '/*') {
                ...
                html += '<span class="cc">' + escape(chunk) + '</span>';
                i += chunk.length;
                continue;
            }
            // Line comment // ...
            ...
            if (code[i] === '"') {
                ...
                if (j < len) j++;
                html += '<span class="cs">' + escape(code.slice(i, j)) + '</span>';
                i = j;
                continue;
            }
            // Attribute [ ... ]
            if (code[i] === '[') {
                ...
                html += '<span class="ca">' + escape(code.slice(i, j)) + '</span>';
                i = j;
                continue;
            }
            // Word (identifier / keyword / type / number)
            const wordMatch = code.slice(i).match(/^([a-zA-Z_][a-zA-Z0-9_]*|\d+\.?\d*(?:f|F)?)/);
            if (wordMatch) {
                const word = wordMatch[1];
                const numMatch = word.match(/^\d/);
                if (numMatch) {
                    html += '<span class="cn">' + escape(word) + '</span>';
                } else if (keywords.has(word)) {
                    html += '<span class="ck">' + escape(word) + '</span>';
                } else if (types.has(word)) {
                    html += '<span class="ct">' + escape(word) + '</span>';
                } else {
                    html += escape(word);
                }
                i += word.length;
    ...
    renderGuide() {
        ...
        const totalGuides = unityGuideData.reduce((sum, cat) => sum + cat.guides.length, 0);
        const totalSteps = unityGuideData.reduce((sum, cat) => sum + cat.guides.reduce((s, g) => s + (g.steps || []).length, 0), 0);
        const diffCounts = { 'Başlangıç': 0, 'Orta': 0, 'İleri': 0 };
        unityGuideData.forEach(cat => cat.guides.forEach(g => { if (diffCounts[g.difficulty] !== undefined) diffCounts[g.difficulty]++; }));
            ...
            <div class="guide-welcome">
                ...
                <div class="guide-stats">
                    <div class="guide-stat-item">
                        <span class="guide-stat-number">${unityGuideData.length}</span>
        ...
        unityGuideData.forEach(category => {
        ...
        const doSearch = () => {
            ...
            if (q) {
                matches = matches.filter(g => {
                    const stepsContent = (g.steps || []).map(s => s.title + ' ' + (s.content || '')).join(' ');
                    const codeFlowContent = (g.codeFlow || []).map(cf => (cf.title || '') + ' ' + (cf.desc || '') + ' ' + (cf.code || '')).join(' ');
                    const haystack = normalize(g.title + ' ' + (g.description || '') + ' ' + stepsContent + ' ' + codeFlowContent + ' ' + g.categoryName);
    ...
    showGuideDetail(guide) {
        ...
        let codeFlowHtml = '';
        if (guide.codeFlow && guide.codeFlow.length > 0) {
            codeFlowHtml = `<div class="guide-codeflow"><h3>💻 C# Kod Örnekleri</h3>`;
            codeFlowHtml += guide.codeFlow.map(cf => {
                let varsHtml = '';
                if (cf.variables && cf.variables.length > 0) {
                    varsHtml = `<div class="bp-vars-table"><h5>📌 Değişkenler</h5><div class="bp-vars-grid">`;
                    varsHtml += cf.variables.map(v =>
                ...
                let eventsHtml = '';
                if (cf.events && cf.events.length > 0) {
                    eventsHtml = `<div class="bp-events-list"><h5>⚡ Kullanılan Event'ler</h5>`;
                    eventsHtml += cf.events.map(e =>
                        `<div class="bp-event-item"><span class="bp-event-name">${e.name}</span><span class="bp-event-desc">${e.desc || ''}</span></div>`
                    ).join('');
                    eventsHtml += `</div>`;
                }
                const tags = (cf.tags || []).map(t =>
                    `<span class="bp-tag bp-tag-${t.type}">${t.label}</span>`
                ).join('');
                const codeHighlighted = this.highlightCSharp(cf.code || '');
                return `<div class="bp-flow-block">
                    <div class="bp-flow-block-header"><h4>${cf.title || 'Kod'}</h4>${tags}</div>
                    ${cf.desc ? `<div class="bp-flow-desc">${cf.desc}</div>` : ''}
                    <div class="code-block-wrapper">
                        <div class="code-block-header">C# — ${cf.title || 'Kod'}</div>
                        <div class="code-block-content">${codeHighlighted}</div>
                    </div>
                    ${varsHtml}${eventsHtml}
                </div>`;
            }).join('');
        ...
        panel.innerHTML = `
            ...
            <div class="guide-steps-container">
                <h3>📋 Adımlar</h3>
                ${stepsHtml || '<p>Bu rehberde adım yok.</p>'}
            </div>
            ${codeFlowHtml}
            ${tipsHtml}
            ${warningsHtml}
    ...
    renderShortcuts() {
        ...
        shortcutData.forEach(section => {
            ...
            const items = section.shortcuts || section.items || [];
            items.forEach(item => {
                const card = document.createElement('div');
                card.className = 'shortcut-card';
                const keyText = item.keys || item.key || '';
                card.innerHTML = `
                    <div class="shortcut-key">${keyText}</div>
